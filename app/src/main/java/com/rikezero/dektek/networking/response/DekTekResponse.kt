package com.rikezero.dektek.networking.response
 ...
 */
sealed class DekTeKResponse<out T> {
     ...
     */
    data class Success<T>(val data: T) : DekTeKResponse<T>()
     ...
     */
    data class Error(val exception: Exception) : DekTeKResponse<Nothing>()
     ...
     * @return Returns the original [DekTeKResponse] to allow for method chaining.
     *
     * Example usage:
     * ```
     * val response: DekTeKResponse<User> = fetchUser()
     ...
     * ```
     */
    inline fun onSuccess(action: (T) -> Unit): DekTeKResponse<T> {
     ...
     * @return Returns the original [DekTeKResponse] to allow for method chaining.
     */
    inline fun onError(action: (Exception) -> Unit): DekTeKResponse<T> {
